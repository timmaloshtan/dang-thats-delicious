...
function typeAhead(search) {
  ...
  searchInput.on('input', function (event) {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (_ref) {
      var data = _ref.data;

      if (data.length) {
        return searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(data));
      }

      searchResults.innerHTML = '\n          <div class="search__result">No results for "' + _dompurify2.default.sanitize(_this.value) + '" found!</div>\n        ';
    }).catch(console.error);
  });
